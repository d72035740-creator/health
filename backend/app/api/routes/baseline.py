...
async def baseline_start():
    try: baseline_service.start(); return baseline_service.snapshot()
...
async def run_demo_calibration():
    ...
    try:
        if baseline_service.snapshot().state.value=="UNINITIALIZED": baseline_service.start()
...
async def baseline_finalize():
    try: baseline_service.finalize(); return baseline_service.snapshot()
    except BaselineError as error: raise HTTPException(status_code=409,detail=str(error)) from error
